      ...
      cardEl.innerHTML =
        ...
        '<div class="card-body">' +
          '<div class="card-inner">' +
            ...
            '</div>' +
            '<div class="section-label" style="margin-top:22px">Resources &amp; where to learn it</div>' +
...
async function boot() {
  ...
  // 3. Wire sign-in button — show info if Firebase not configured yet
    ...
    if (typeof signInWithGoogle === 'function') {
      ...
      try {
        ...
        if (err.code !== 'auth/popup-closed-by-user') {
          showToast('Sign-in failed — check Firebase config in js/auth.js', 'error');
        }
      ...
      showToast('🔥 Fill in FIREBASE_CONFIG in js/auth.js to enable sign-in!', 'success');
    }
  });
  ...
  // 4. Try to init Firebase (async, non-blocking)
    ...
    if (ok && typeof onAuthChange === 'function') {
      onAuthChange(async user => {
        ...
        if (user) {
          ...
          applyCheckedToUI();
          showToast('Welcome back, ' + (user.displayName?.split(' ')[0] || 'there') + '! ☁️ Progress synced.');
        }
      });
    }
  }
